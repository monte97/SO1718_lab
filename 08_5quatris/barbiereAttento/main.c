...
pthread_cond_t   condFine;		/* aggiunta per permetere al cliente di segnalare al barbiere che ha finito */
...
{ 
	...
	while(1) {
		...
		/*prima di salutare devo aspettare che l'altro esca*/
		DBGpthread_cond_wait(&condFine, &mutex, Blabel);
	...
	while(1) {
		...
		if ( numclientiincoda >= NUMSEDIE )  {
			...
			*/
			...
			/*dopo essere stato salutato segnala che se ne va al barbiere*/
			DBGpthread_cond_signal(&condFine, Clabel);
...
{ 
	...
	if( rc ) PrintERROR_andExit(rc,"pthread_cond_init failed");
	rc = pthread_cond_init( &condFine , NULL);
	if( rc ) PrintERROR_andExit(rc,"pthread_cond_init failed");
