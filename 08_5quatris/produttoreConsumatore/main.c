...
/*le condizione sui consumatori gestito come array perchè ce ne sono di due tipi e voglio svegliare solo quello che mi serve davvero*/
pthread_cond_t   condProd, condCons[TIPI]; 
int numBufferPieni=0; /* 0 o 1 */
int numProdWaiting=0;
int numProdWaitingAndSignalled=0; /* deve essere <= #buffer vuoti */
int numConsWaiting[TIPI]={0};
int numConsWaitingAndSignalled[TIPI]={0}; /* deve essere <= #buffer pieni */
int turno = 0; /*Quale tipo di consumatore deve accedere*/
...
{ 
	...
	while(1) {
		...
		*/
		if( 
			( numConsWaitingAndSignalled[turno] < numConsWaiting[turno] ) 
			&&
			( numConsWaitingAndSignalled[turno] < numBufferPieni ) 
		  ) {
			/* risveglio un Consumatore per svuotare un buffer */
			DBGpthread_cond_signal(&condCons[turno],Plabelsignal); 
			/*voglio eseguire a turni alteri i due tipi di consumatori*/
			numConsWaitingAndSignalled[turno]++;
		}
...
{ 
	...
	/*usare indice come info per il tipo di consumatore*/
	indice=*((int*)arg);
	free(arg);
	...
	while(1) {
		...
		*/
		if( numConsWaitingAndSignalled[turno] >= numBufferPieni ) {

#ifdef DEBUGSTAMPAAZIONI
			printf("Cons %s ENTRA CODA\n",Clabel);fflush(stdout);
#endif

			numConsWaiting[turno]++;
			DBGpthread_cond_wait(&condCons[turno],&mutex,Clabel);
			numConsWaiting[turno]--;
			numConsWaitingAndSignalled[turno]--;
		}
#ifdef DEBUGSTAMPAAZIONI
		...
		val=valGlobale;
		/*un cliente si è servito quindi alterno il tipo*/
		turno = (turno + 1) % TIPI;
...
{ 
	...
	rc = pthread_cond_init(&condCons[0], NULL);
	if( rc ) PrintERROR_andExit(rc,"pthread_cond_init failed");
	rc = pthread_cond_init(&condCons[1], NULL);
	if( rc ) PrintERROR_andExit(rc,"pthread_cond_init failed");
