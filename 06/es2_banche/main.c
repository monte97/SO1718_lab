...
pthread_mutex_t mutexdata[NBANK]; /**/
...
void *preleva(void* arg){
	...
	while(1){ /*ogni thread deve rappresentare un loop*/
		...
		pthread_mutex_lock(&mutexdata[index]); /*effettuo il blocco sulla risorsa condivisa*/	
			printf("Banca %d preleva\n", index);
			fflush(stdout); /*forzo svuotaggio buffer per essere sicuro che il messaggio venga stampato asap*/	
			banks[index].B -= 9;/*accedo alla risorsa condivisa*/ 
			banks[index].N++;
			sleep(1);
		pthread_mutex_unlock(&mutexdata[index]); /*rilascio la risorsa condivisa*/
	}
	pthread_exit(NULL);
}
...
void *deposita(void* arg){
	...
	while(1){
		sleep(1);
		pthread_mutex_lock(&mutexdata[index]);
			printf("Banca %d deposita\n", index);
			fflush(stdout);
			banks[index].B += 10;
			banks[index].B++;
			sleep(1);
		pthread_mutex_unlock(&mutexdata[index]);
	}
	pthread_exit(NULL);
}
...
int main(void) {
	...
	for (i = 0; i < NBANK; i++){
		pthread_mutex_init(&mutexdata[i], NULL); /*inizializzo il lock sulla risorsa condivisa*/
	}
